import os
import pandas as pd
import numpy as np
import sklearn
from sklearn.preprocessing import LabelEncoder, MinMaxScaler, OneHotEncoder
from sklearn.model_selection import train_test_split
from deepctr import SingleFeat


def load_criteo(raw_path, test_size=0.1):
    data = pd.read_csv(raw_path, header=None, sep='\t')

    exceptions = [16, 17, 25, 29, 34]
    
    sparse_feature = [i for i in range(14, 40) if i not in exceptions]
    dense_feature = [i for i in range(1, 14)]

    data[sparse_feature] = data[sparse_feature].fillna('-1', )
    data[dense_feature] = data[dense_feature].fillna(0, )
    target = 0
    
    print(data.shape, data[sparse_feature].shape, data[dense_feature].shape)

    for feat in sparse_feature:
        lbe = LabelEncoder()
        data[feat] = lbe.fit_transform(data[feat])
    mms = MinMaxScaler(feature_range=(0, 1))
    data[dense_feature] = mms.fit_transform(data[dense_feature])

    sparse_feature_list = [SingleFeat('C' + str(feat), data[feat].nunique()) for feat in sparse_feature]
    dense_feature_list = [SingleFeat('I' + str(feat), 0) for feat in dense_feature]    

    print(sparse_feature_list)
    print(dense_feature_list)

    train, test = train_test_split(data, test_size=test_size, random_state=666)
    train_model_input = [train[feat].values for feat in sparse_feature] + \
                        [train[feat].values for feat in dense_feature]
    test_model_input = [test[feat].values for feat in sparse_feature] + \
                       [test[feat].values for feat in dense_feature]

    return train_model_input, train[target].values, \
           test_model_input, test[target].values, \
           sparse_feature_list, dense_feature_list


def load_avazu(raw_path, test_size=0.1):
    data = pd.read_csv(raw_path)

    sparse_feature = ['hour', 'C1', 'banner_pos', 'site_id','site_domain', 'site_category', 'app_id', 'app_domain', 'app_category', 'device_model', 'device_type', 'device_conn_type']
    dense_feature = ['C' + str(i) for i in range(14, 22)]

    data[sparse_feature] = data[sparse_feature].fillna('-1', )
    data[dense_feature] = data[dense_feature].fillna(0, )
    target = ['click']
    
    print(data.shape, data[sparse_feature].shape, data[dense_feature].shape)

    for feat in sparse_feature:
        lbe = LabelEncoder()
        data[feat] = lbe.fit_transform(data[feat])
    mms = MinMaxScaler(feature_range=(0, 1))
    data[dense_feature] = mms.fit_transform(data[dense_feature])

    sparse_feature_list = [SingleFeat(feat, data[feat].nunique()) for feat in sparse_feature]
    dense_feature_list = [SingleFeat(feat, 0) for feat in dense_feature]    

    print(sparse_feature_list)
    print(dense_feature_list)

    train, test = train_test_split(data, test_size=test_size, random_state=666)
    #train, test = train_test_split(data, test_size=test_size, shuffle=False)
    train_model_input = [train[feat].values for feat in sparse_feature] + \
                        [train[feat].values for feat in dense_feature]
    test_model_input = [test[feat].values for feat in sparse_feature] + \
                       [test[feat].values for feat in dense_feature]

    return train_model_input, train[target].values, \
           test_model_input, test[target].values, \
           sparse_feature_list, dense_feature_list


def load_frappe(raw_path, test_size=0.1):
    data = pd.read_csv(raw_path, header=None)

    sparse_feature = list(range(1,11))
    dense_feature = []

    data[sparse_feature] = data[sparse_feature].fillna('-1', )
    data[dense_feature] = data[dense_feature].fillna(0, )
    target = 0

    print(data.shape, data[sparse_feature].shape, data[dense_feature].shape)

    # for feat in sparse_feature:
    #     lbe = LabelEncoder()
    #     data[feat] = lbe.fit_transform(data[feat])
    # mms = MinMaxScaler(feature_range=(0, 1))
    # data[dense_feature] = mms.fit_transform(data[dense_feature])

    sparse_feature_list = [SingleFeat('C' + str(feat), data[feat].nunique()) for feat in sparse_feature]
    dense_feature_list = [SingleFeat('I' + str(feat), 0) for feat in dense_feature]

    print(sparse_feature_list)
    print(dense_feature_list)

    train, test = train_test_split(data, test_size=test_size, shuffle=False)
    train_model_input = [train[feat].values for feat in sparse_feature] + \
                        [train[feat].values for feat in dense_feature]
    test_model_input = [test[feat].values for feat in sparse_feature] + \
                       [test[feat].values for feat in dense_feature]

    print(train)
    print(train[target].values)

    return train_model_input, train[target].values, \
           test_model_input, test[target].values, \
           sparse_feature_list, dense_feature_list


if __name__ == "__main__":
    pass


