...
def build_msg(stage, epoch, **kwargs):
    """build message"""
    def msg_token(key, val):
        if isinstance(val, float) or isinstance(val, np.float32):
            return "{}:{:.6f}".format(key, val)
        else:
            return "{}:{:d}".format(key, val)

    # build msg
    time = datetime.now().isoformat()[5:22]
    prefix = "" if stage == "Trn" else "==> "
    suffix = "" if stage == "Trn" else " <=="
    msg = "{}[{},{}] ep:{} ".format(prefix, stage, time, epoch)

    str_dict = [msg_token(k, v) for k, v in kwargs.items()]
    # str_dict = ["{}:{:.6f}".format(k, v) for k, v in kwargs.items()]
    msg += " ".join(str_dict)
    msg += suffix
